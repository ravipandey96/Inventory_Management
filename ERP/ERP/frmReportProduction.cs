...
namespace ERP
{
    public partial class frmReportProduction: Form
    { 
        bool drag = false;
...
/*            else
        ...
        private void btnShow5_Click(object sender , EventArgs e)
        {
            ...
            lblWait5.Visible = true;
                ...
                string qry = "select DATENAME(month,m.Date) as Date,f.thickness as Thickness,sum(m.qty) as Qty,isnull((select sum(r_qty) from Consumption c where c.RNo=1 and DATENAME(month,m.Date)=DATENAME(month,c.Date) and c.FNo=m.FNo),0) as BasePaper,isnull((select sum(r_qty) from Consumption c where c.RNo=2 and DATENAME(month,m.Date)=DATENAME(month,c.Date) and c.FNo=m.FNo),0) as BOPP,isnull((select sum(r_qty) from Consumption c where c.RNo=3 and  DATENAME(month,m.Date)= DATENAME(month,c.Date) and c.FNo=m.FNo),0) as ColourInk,isnull((select sum(r_qty) from Consumption c where c.RNo=4 and DATENAME(month,m.Date)= DATENAME(month,c.Date) and c.FNo=m.FNo),0) as Formaldehyde,isnull((select sum(r_qty) from Consumption c where c.RNo=5 and DATENAME(month,m.Date)= DATENAME(month,c.Date) and c.FNo=m.FNo),0) as KraftPaper,isnull((select sum(r_qty) from Consumption c where c.RNo=6 and DATENAME(month,m.Date)= DATENAME(month,c.Date) and c.FNo=m.FNo),0) as Melamine,isnull((select sum(r_qty) from Consumption c where c.RNo=7 and DATENAME(month,m.Date)= DATENAME(month,c.Date) and c.FNo=m.FNo),0) as Methanol,isnull((select sum(r_qty) from Consumption c where c.RNo=8 and DATENAME(month,m.Date)= DATENAME(month,c.Date) and c.FNo=m.FNo),0) as Phenol from v_manufacturing m left join finished f on f.sno=m.fno where (" + VDate + " >= '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate + " <= '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "') group by  DATENAME(month,m.Date),f.Thickness,m.FNo";
                List<clsProductionReport> list = db.Query<clsProductionReport>(qry, commandType: CommandType.Text).ToList();
                    ReportDocument report = new ReportDocument();
                    //list4 = list.Concat(list1).Concat(list2).Concat(list3).ToList();
                    report.Load(Globals.SoftPath + "\\Report\\rptProductionMonthly.rpt");
                    report.SetDataSource(list);
