...
namespace ERP
{
    public partial class frmFinishedParticular : Form
    { 
        bool drag = false;
...
/*            else
        ...
        {
            // Itemwise opening stock,production,sale,return,closing
            this.Enabled = false;
            this.Cursor = Cursors.WaitCursor;
            ...
            using (IDbConnection db = new SqlConnection(Globals.iniConnectionString))
            {
                ...
                if (uctxtFNo.txt.Text.Trim() != "")
                {
                    ...
                    List<clsFinishedParticular> list = db.Query<clsFinishedParticular>(qry, commandType: CommandType.Text).ToList();
                    ReportDocument report = new ReportDocument();
                    //list4 = list.Concat(list1).Concat(list2).Concat(list3).ToList();
                    report.Load(Globals.SoftPath + "\\Report\\rptFinishedParticular.rpt");
                    report.SetDataSource(list);
