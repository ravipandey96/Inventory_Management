...
namespace ERP
{
    public partial class frmTaxCategory : Form
    {
        bool drag = false;
            ...
            try
            {
                ...
                if (status == "add")
                {
                    ...
                    else
                    {
                        ...
                        sql = "insert into m_master(SNo,MType,Mname,TaxType,CGST,SGST,IGST,Cess,TaxOnMRP) values(" +
        ...
        {
            string SNum = "Row_Number() over(order by L.Mname) as [S.No.]";
            sql = "Select L.SNo," + SNum + ",L.Mname as [Tax Name],L.TaxType as [Tax Type],L.CGST,L.SGST,L.IGST from m_master L where L.MType='" + this.Text + "' and L.Mname like '%" + uSearch.txt.Text.Trim() + "%' order by L.Mname";
            cmd = new SqlCommand(sql , Globals.con);
        ...
        {
            sql = "Select L.SNo,L.MType,L.Mname,L.TaxType,L.CGST,L.SGST,L.IGST,L.Cess,L.TaxOnMRP from m_master L where L.MType='" + this.Text + "' and L.SNo=" + int.Parse(uctxtSNo.txt.Text) + "";
            cmd = new SqlCommand(sql , Globals.con);
            ...
            while (rd.Read())
            {
                ...
                uTax.txt.Text = rd["TaxOnMRP"].ToString();
            }
            cmd.Dispose();
            rd.Close();
        }
        private void uSearch_TextChanged(object sender , EventArgs e)
        {
            ...
            dv.RowFilter = "[Tax Name] like '%" + uSearch.txt.Text + "%'";
            gList.DataSource = dv;
            if (gList.Rows.Count > 0)
            {
                CurrentRow = 0;
        ...
        private void uSearchPrimary_Load(object sender , EventArgs e)
        {
            try
            {
                sql = "Select SNo,Mname from m_master where MType='GSTType' order by Mname";
                cmd = new SqlCommand(sql , Globals.con);
                rd = cmd.ExecuteReader();
                uSearchGSTType.lstName.Items.Clear();
                while (rd.Read())
                {
                    ucSearchList.MList lst = new ucSearchList.MList();
                    lst.Code = int.Parse(rd["SNo"].ToString());
                    lst.Name = rd["Mname"].ToString();
                    uSearchGSTType.lstName.Items.Add(lst);
                }
                uSearchGSTType.lstName.ClearSelected();
            }
            catch (Exception ex)
            {
                Globals.msgbox(this , ex.Message);
        ...
        {
            uSGST.txt.Text = uCGST.txt.Text;
            uIGST.txt.Text = (val(uCGST.txt.Text) + val(uSGST.txt.Text)).ToString();
        }
