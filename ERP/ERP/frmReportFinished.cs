...
namespace ERP
{
    public partial class frmReportFinished : Form
    { 
        bool drag = false;
        ...
        private void frmDemo2_Load(object sender , EventArgs e)
        {
            ...
            pnlFStock.Left = pnlAdvance.Left;
            pnlFStock.Top = pnlAdvance.Top;
...
/*            else
        ...
        private void btnShow5_Click(object sender , EventArgs e)
        {
            ...
            lblWait5.Visible = true;
                ...
                //string qry = "select r.SNo,r.Item_Name,sum(io.opstock) as Opening_Stock from m_raw r left join m_item_opening io on r.sno=io.INo where io.Item_Type='Raw Material' and (" + VDate + " >='" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate + "<='" + uTo5.date.Value.ToString("yyyy-MM-dd") + "') group by r.sno,r.Item_Name order by r.Item_Name";
               //string qry1 = "select r.SNo,r.Item_Name,isnull((select sum(op.OpStock) from m_Item_Opening op where op.Item_type = 'Raw Material' and op.INo = r.SNo and (" + VDate + " >='" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate + "<='" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as OPSTOCK,isnull((select sum(p.qty) from v_purchase p where p.RNo = r.SNo and (" + VDate1 + " >='" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate1 + "<='" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as PURCHASE,isnull((select sum(pr.qty) from Purchase_Return pr  where pr.RNo = r.SNo and (" + VDate2 + " >='" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate2 + "<='" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as purReturn,isnull((select sum(c.R_Qty) from Consumption c where C.RNo = r.SNo and (" + VDate3 + " >='" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate3 + "<='" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as CONSUMED from m_raw r group by r.SNo,r.Item_Name order by r.SNo";
                string qry = "select f.sno,f.thickness,isnull((select sum(op.OpStock) from m_Item_Opening op where op.Item_type = 'Finished Good' and op.INo = f.SNo and (" + VDate + " >= '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate + " <= '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as OPSTOCK, isnull((select sum(vm.qty) from v_manufacturing vm where vm.FNo = f.SNo and (" + VDate1 + " >= '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate1 + " <= '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as PRODUCTION,isnull((select sum(vm.breackage) from v_manufacturing vm where vm.FNo = f.SNo and(" + VDate1 + " >= '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate1 + " <= '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as BREACKAGE,isnull((select sum(sr.Total) from Sales_Return sr where sr.FNo = f.SNo and(" + VDate2 + " >= '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate2 + " <= '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as SaleReturn,isnull((select sum(s.Total_Qty) from sales s where s.FNo = f.SNo and(" + VDate3 + " >= '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate3 + " <= '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as Sales,isnull((select sum(s.ISI)   from sales s where s.FNo = f.SNo and (" + VDate3 + " >= '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate3 + " <= '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as ISI,isnull((select sum(s.taxable_value) from sales s where s.FNo = f.SNo and (" + VDate3 + " >= '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate3 + " <= '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as Value,isnull((select sum(s.SGST) from sales s where s.FNo = f.SNo and(" + VDate3 + " >= '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate3 + " <= '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as SGST,isnull((select sum(s.CGST) from sales s where s.FNo = f.SNo and (" + VDate3 + " >= '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate3 + " <= '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as CGST,isnull((select sum(s.IGST) from sales s where s.FNo = f.SNo and(" + VDate3 + " >= '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate3 + " <= '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')),0) as IGST,'0' as CLSTOCK from finished f group by f.sno,f.thickness order by f.SNo";
                List<clsFinishedReport> list = db.Query<clsFinishedReport>(qry, commandType: CommandType.Text).ToList();
                    ReportDocument report = new ReportDocument();
                    //list4 = list.Concat(list1).Concat(list2).Concat(list3).ToList();
                    report.Load(Globals.SoftPath + "\\Report\\rptFinished.rpt");
                    report.SetDataSource(list);
