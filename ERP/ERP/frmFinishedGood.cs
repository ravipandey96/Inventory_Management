...
namespace ERP
{
    public partial class frmFinishedGood : Form
    {
        bool drag = false;
        ...
        {
            if (uctxtName.txt.Text.Trim() == "")
            {
                Globals.msgbox(this, " Please Enter Item Name.");
                uctxtName.Focus();
                return;
            }
            if (usUnit.txt.Text.Trim() == "")
            {
                Globals.msgbox(this, " Please Enter Unit Name.");
                usUnit.Focus();
                return;
            }
            if (usTaxType.txt.Text.Trim() == "")
            {
                Globals.msgbox(this, " Please Enter Tax Type.");
                usTaxType.Focus();
                return;
            }
            ...
            using (SqlConnection con = new SqlConnection(Globals.iniConnectionString))
            {
                ...
                try
                {
                    ...
                    if (status == "add")
                    {
                        if (Globals.chkRecord("finished", "Item_Name='" + uctxtName.txt.Text.Trim() + "'"))
                        {
                            if (Globals.msgbox(this, "This Employee Name Already Exists \n Do You Save ?", "YesNo") != 1)
                            {
                                uctxtName.Focus(); btnSave.Enabled = true; return;
                            }
                        }
                        uctxtSNo.txt.Text = Globals.MaxCode("finished", "SNo").ToString();
                        long sno = long.Parse(uctxtSNo.txt.Text);
                        sql = "insert into finished(SNo,Item_Name,Alias,PrintName,Unit,Thickness,Grade,Rate,HSNCODE,TaxType,CGST,SGST,IGST,Cess) values(" +
                        sno
                        + ",'" + uctxtName.txt.Text
                        + "','" + uctxtAlias.txt.Text
                        + "','" + uctxtPrint.txt.Text
                    ...
                    {
                        sql = "update finished set Item_Name='" + uctxtName.txt.Text
        ...
        {
            //int i = 0;
            sql = "Select f.SNo,Row_Number() over(order by Item_Name) as [S.No.],Item_Name as [Item Name],Thickness,m.MName as Unit,Rate,HSNCODE as HSN,f.TaxType from finished f left join m_master m on m.MType='ItemUnit' and f.Unit=m.SNo where Item_Name like '%" + uSearch.txt.Text.Trim() + "%' order by Item_Name";
        ...
        {
            sql = "Select f.*,m.MName as Uname from finished f left join m_master m on f.Unit=m.SNo where f.SNo=" + (uctxtSNo.txt.Text) + "";
        ...
        private void uSearch_TextChanged(object sender, EventArgs e)
        {
            ...
            dv.RowFilter = "[Item Name] like '%" + uSearch.txt.Text + "%' or [Thickness] like '%" + uSearch.txt.Text + "%'";
            gList.DataSource = dv;
            if (gList.Rows.Count > 0)
            {
                CurrentRow = 0;
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (gList.Rows.Count > 0 && (CurrentRow >= 0 && CurrentRow < gList.Rows.Count))
            {
                ...
                {
                    Globals.DeleteRecord("finished", gList.Rows[CurrentRow].Cells[0].Value);
                    btnShow_Click(sender, e);
                }
            }
        }
     ...
     //       string[] post = { "GST FREE", "Nil-Rated","GST 5%", "GST 12%","GST 18%","GST 28%" };
        ...
        private void grdRatio_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (gList.Rows.Count > 0)
