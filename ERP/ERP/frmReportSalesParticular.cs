...
namespace ERP
{
    public partial class frmReportSalesParticular : Form
    { 
        bool drag = false;
        ...
        //-----END
        public frmReportSalesParticular()
        {
            InitializeComponent();
        }
        ...
        private void frmDemo2_Load(object sender , EventArgs e)
        {
            ...
            pnlsales.Left = pnlAdvance.Left;
            pnlsales.Top = pnlAdvance.Top;
...
/*            else
        ...
        private void btnShow5_Click(object sender , EventArgs e)
        {
            ...
            lblWait5.Visible = true;
                ...
                string qry = "select t.SNo,s.Sdate,s.Total_Regular,s.Total_ISI,'0' as Total,t.Batch_No,isnull((select sum(ps.Regular) from Pstock ps where ps.FNo = " + val(uctxtFNo.txt.Text) + " and ps.PDate = s.Sdate),0) as PRegular,isnull((select sum(ps.BIS) from Pstock ps where ps.FNo = " + val(uctxtFNo.txt.Text) + " and ps.PDate = s.Sdate),0) as PBIS,isnull((select sum(ps.Breackage) from Pstock ps where ps.FNo = " + val(uctxtFNo.txt.Text) + " and ps.PDate = s.Sdate),0) as Breackage,'0' as Mfd,'0' as Opening_Bal,ss.BillNo,t.Vehicle_No,ss.Regular,ss.BIS,'0' as Sold,m.SName,'0' as Closing_bal,f.Thickness as Item_Name from sales s left join m_party m on s.cno = m.SNo left join finished f on s.FNo = f.SNo,trans_1 t, Sstock ss where s.FNo = " + val(uctxtFNo.txt.Text) + " and(s.VNo = t.SNo and t.VType = 'sales') and (ss.Vno = s.VNo and ss.FNo = s.FNo)  and (" + VDate3 + " >='" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and " + VDate3 + "<='" + uTo5.date.Value.ToString("yyyy-MM-dd") + "')";
                List<clsSales_RegisterParticular> list = db.Query<clsSales_RegisterParticular>(qry, commandType: CommandType.Text).ToList();
                    ReportDocument report = new ReportDocument();
                    //list4 = list.Concat(list1).Concat(list2).Concat(list3).ToList();
                    report.Load(Globals.SoftPath + "\\Report\\rptSalesRegisterParticular.rpt");
                    report.SetDataSource(list);
        ...
        private void usParticular_Leave_1(object sender, EventArgs e)
        {
        ...
        private void frmReportSalesParticular_Activated(object sender, EventArgs e)
        {
