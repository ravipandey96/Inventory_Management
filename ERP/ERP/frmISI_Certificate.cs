...
namespace ERP
{
    public partial class frmISI_Certificate : Form
    { 
        bool drag = false;
        ...
        private void frmDemo2_Load(object sender , EventArgs e)
        {
            ...
            pnlISI.Left = pnlAdvance.Left;
            pnlISI.Top = pnlAdvance.Top;
...
/*            else
        ...
        private void btnShow5_Click(object sender , EventArgs e)
        {
            ...
            lblWait5.Visible = true;
                ...
                using (IDbConnection db = new SqlConnection(Globals.iniConnectionString))
                {
                    ...
                    string VDate2 = "(DATEADD(DAY, DATEDIFF(day, 0, pr.Date), 0))";
                    string VDate3 = "(DATEADD(DAY, DATEDIFF(day, 0, c.Date), 0))";
                    string qry = "select s.VNo,s.Sdate,s.Bill_no,s.ISI,t1.Batch_no,t1.Vehicle_No,f.Item_name,p.SName from sales s left join trans_1 t1 on s.VNo=t1.SNo left join finished f on s.FNo=f.SNo left join m_party p on s.CNo=p.SNo where s.FNo="+uctxtFNo.txt.Text+ " and t1.VType='sales' and s.Sdate between '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and  '" + uTo5.date.Value.ToString("yyyy-MM-dd")+"' and s.ISI>0";                
                    List<clsSales_RegisterParticular> list = db.Query<clsSales_RegisterParticular>(qry, commandType: CommandType.Text).ToList();
                    ReportDocument report = new ReportDocument();
                    //list4 = list.Concat(list1).Concat(list2).Concat(list3).ToList();
                    report.Load(Globals.SoftPath + "\\Report\\rptISI.rpt");
                    report.SetDataSource(list);
        ...
        private void frmISI_Certificate_Activated(object sender, EventArgs e)
        {
