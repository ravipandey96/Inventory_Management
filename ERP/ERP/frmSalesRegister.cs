...
namespace ERP
{
    ...
    { 
        ...
        private void frmDemo2_Load(object sender , EventArgs e)
        {
            ...
            pnlSalesRegister.Left = pnlAdvance.Left;
            pnlSalesRegister.Top = pnlAdvance.Top;
...
/*            else
        ...
        private void btnShow5_Click(object sender , EventArgs e)
        {
            ...
            lblWait5.Visible = true;
                ...
                //    string VDate = "(DATEADD(DAY, DATEDIFF(day, 0, v.Rdate), 0))";
                ...
                //    string qry = "select Convert(varchar,s.Sdate,103) as inv_date,s.Bill_no as Invoice_no,m.SName as PartyName,f.Item_Name as Item_Name,s.Qty,s.Taxable_Value as Value,s.Transit_Insurance,s.Other_Charges,s.CGST,s.SGST,s.IGST,s.Cess,s.Freight,s.TCS,'0' as Total from sales s,m_party m, finished f where s.FNo = f.SNo and s.CNo = m.SNo and s.Sdate between '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "' order by s.Sdate asc";                
                    string qry = "select t.SNo,Convert(varchar,t.VDate,103) as inv_date,t.Inv_No as Invoice_no,m.SName as PartyName,f.Thickness as Item_Name,s.ISI,IsNull(s.Qty,0) as Qty,s.Taxable_Value as value,s.Transit_Insurance,s.Other_Charges,t.cgst,t.sgst,t.igst,t.TCS,s.Cess,s.Freight,s.Total,'0' as t_Qty from sales s, trans_1 t ,finished f, m_party m where (t.VType = 'sales') and(s.VNo = t.SNo) and (t.PartySNo = m.SNo) and s.FNo = f.SNo and t.VDate between '" + uFrom5.date.Value.ToString("yyyy-MM-dd") + "' and '" + uTo5.date.Value.ToString("yyyy-MM-dd") + "' order by t.VDate,t.SNo,s.sno";
                    List<clsSales_Register> list = db.Query<clsSales_Register>(qry, commandType: CommandType.Text).ToList();
                    ReportDocument report = new ReportDocument();
                    //list4 = list.Concat(list1).Concat(list2).Concat(list3).ToList();
                    report.Load(Globals.SoftPath + "\\Report\\rptSalesRegister.rpt");
                    report.SetDataSource(list);
